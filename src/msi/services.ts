  ...
  return $('.detail-item[data-lectureday]')
    .map((_, element) => {
      ...
      return {
        ...
        ...(heightPercent
          ? { heightPercent: Number.parseFloat(heightPercent) }
          : {})
      };
    })
    .get()
...
function parseTimetablePage(html: string): MsiTimetableResult {
  ...
  const termOptions: MsiTimetableTermOption[] = $('form[name="form1"] select[name="smt"] option')
    ...
    .get();
  const selectedTerm =
    termOptions.find((option) => option.selected) ?? termOptions[0];
...
function parseHistorySummaryMetrics(text: string): {
  ...
  const requestedCredits = requestedMatch?.[1]
    ? parseNumber(requestedMatch[1])
    : undefined;
  const earnedCredits = earnedMatch?.[1]
    ? parseNumber(earnedMatch[1])
    : undefined;
  const totalPoints = totalPointsMatch?.[1]
    ? parseNumber(totalPointsMatch[1])
    : undefined;
  ...
  const table = $("table")
    .filter((_, element) => {
      const headers = $(element)
        ...
        .get();
      return headers.join("|") ===
        "년도|학기|이수구분|교과목명|교과코드|학점|성적|중복코드";
    })
    .first();
...
function parseGradeHistoryPage(html: string): MsiGradeHistoryResult {
  ...
  const overviewCard = $(".card-item.basic")
    .filter((_, card) => cleanText($(card).children(".data-title").first().text()) === "전체취득학점")
    .first();
  const creditCard = $(".card-item.basic")
    .filter((_, card) => cleanText($(card).children(".data-title").first().text()) === "이수구분별 취득학점")
    .first();
  ...
  overviewCard.find("thead th").each((index, th) => {
    ...
    const value = cleanText(
      overviewCard.find("tbody td").eq(index).text()
    );
    if (label) {
...
function parseGraduationPage(html: string): MsiGraduationRequirementsResult {
  ...
  const personalCard = $(".card-item.basic")
    .filter((_, card) => cleanText($(card).children(".data-title").first().text()) === "기본인적사항")
    .first();
  const earnedCard = $(".card-item.basic")
    .filter((_, card) => cleanText($(card).children(".data-title").first().text()) === "이수학점내역")
    .first();
  const requiredCard = $(".card-item.basic")
    .filter((_, card) => cleanText($(card).children(".data-title").first().text()) === "졸업필요학점계")
    .first();
  const notes = $("ol li")
  ...
  const earnedMap = new Map(
    earnedCredits.map((item) => [item.label, item.credits])
  );
  const requiredMap = new Map(
    requiredCredits.map((item) => [item.label, item.credits])
  );
...
export async function getMsiTimetable(
  ...
  if (options.year !== undefined || options.termCode !== undefined) {
    ...
    const queryResponse = await client.postForm(
      new URL(action, MSI_BASE).toString(),
      {
        ...fields,
        ...(options.year !== undefined ? { year: String(options.year) } : {}),
        ...(options.termCode !== undefined
          ? { smt: String(options.termCode) }
          : {})
      }
    );
